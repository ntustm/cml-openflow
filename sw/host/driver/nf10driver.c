...
MODULE_DESCRIPTION("nf1_cml openflow driver");
...
static int __devinit nf10_probe(struct pci_dev *pdev, const struct pci_device_id *id){
	...
	if((err = pci_enable_device(pdev))) {
		printk(KERN_ERR "nf1_cml: Unable to enable the PCI device!\n");
        ret = -ENODEV;
		goto err_out_none;
	}
    ...
    if(dma_set_mask(&pdev->dev, DMA_BIT_MASK(64)) < 0){
        printk(KERN_ERR "nf1_cml: dma_set_mask fail!\n");
        ret = -EFAULT;
    ...
    if(pci_enable_msi(pdev) != 0){
        printk(KERN_ERR "nf1_cml: failed to enable MSI interrupts\n");
        ret = -EFAULT;
		goto err_out_clear_master;
    }
	
    // be nice and tell kernel that we'll use this resource
	printk(KERN_INFO "nf1_cml: Reserving memory region for NF10\n");
	if (!request_mem_region(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0), DEVICE_NAME)) {
		printk(KERN_ERR "nf1_cml: Reserving memory region failed\n");
        ret = -ENOMEM;
        goto err_out_msi;
	}
	...
	if (card == NULL) {
		printk(KERN_ERR "nf1_cml: Private card memory alloc failed\n");
		ret = -ENOMEM;
    ...
    *(((uint64_t*)card->cfg_addr)+30) = 1;
    ...
    if( (card->host_rx_dne_ptr == NULL) ||
        ...
        printk(KERN_ERR "nf1_cml: cannot allocate dma buffer\n");
    ...
    *(((uint64_t*)card->cfg_addr)+19) = card->rx_dne_mask;
    ...
    if(!(card->wq = create_workqueue("int_hndlr"))){
        printk(KERN_ERR "nf1_cml: workqueue failed\n");
    }
    ...
    if(card->tx_bk_skb == NULL || card->tx_bk_dma_addr == NULL || card->tx_bk_size == NULL || card->tx_bk_port == NULL ||
       card->rx_bk_skb == NULL || card->rx_bk_dma_addr == NULL || card->rx_bk_size == NULL){
        printk(KERN_ERR "nf1_cml: kmalloc failed");
        goto err_out_free_private2;
    }
    ...
    //     printk(KERN_INFO "nf1_cml: AEL2005 PHY chips are configured\n");
    // }
    // else {
    //     printk(KERN_INFO "nf1_cml: AEL2005 PHY chips were already configured\n");
    // }

    // success
    ret = nf10iface_probe(pdev, card);
    if(ret < 0){
        printk(KERN_ERR "nf1_cml: failed to initialize interfaces\n");
        goto err_out_free_private2;
    }

    ret = nf10fops_probe(pdev, card);
    if(ret < 0){
        printk(KERN_ERR "nf1_cml: failed to initialize dev file\n");
        goto err_out_free_private2;
    }
    else{
        printk(KERN_INFO "nf1_cml: device ready\n");
        return ret;
    }
...
static void __devexit nf10_remove(struct pci_dev *pdev){
    ...
    // free private data
    printk(KERN_INFO "nf1_cml: releasing private memory\n");
    ...
    // release memory
    printk(KERN_INFO "nf1_cml: releasing mem region\n");
	release_mem_region(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0));
	release_mem_region(pci_resource_start(pdev, 2), pci_resource_len(pdev, 2));

    // disabling device
    printk(KERN_INFO "nf1_cml: disabling device\n");
    pci_disable_msi(pdev);
    pci_clear_master(pdev);
	pci_disable_device(pdev);
}
...
pci_ers_result_t nf10_pcie_error(struct pci_dev *dev, enum pci_channel_state state){
    printk(KERN_ALERT "nf1_cml: PCIe error: %d\n", state);
...
{
	printk(KERN_INFO "nf1_cml: module loaded\n");
...
static void __exit nf10_exit(void)
{
	...
	printk(KERN_INFO "nf1_cml: module unloaded\n");
}
