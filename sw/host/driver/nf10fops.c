...
long nf10fops_ioctl (struct file *f, unsigned int cmd, unsigned long arg){
    ...
    case NF10_IOCTL_CMD_READ_STAT:
        if(copy_from_user(&addr, (uint64_t*)arg, 8)) printk(KERN_ERR "nf1_cml: ioctl copy_from_user fail\n");
        if(addr >= 4096/8) return -EINVAL;
        val = *(((uint64_t*)card->cfg_addr) + 4096/8 + addr);
        if(copy_to_user((uint64_t*)arg, &val, 8))  printk(KERN_ERR "nf1_cml: ioctl copy_to_user fail\n");
        break;
    case NF10_IOCTL_CMD_WRITE_REG:
        ...
        if(axi_wr_cnt > 64){
            printk(KERN_ERR "nf1_cml: AXI write buffer full\n");
            return -EFAULT;
        }
    ...
    case NF10_IOCTL_CMD_READ_REG:
        ...
        *(((uint64_t*)card->cfg_addr) + 129) = (addr << 32);
        ...
        break;
    default:
        printk(KERN_ERR "nf1_cml: unknown ioctl\n");
        break;
    }
...
int nf10fops_probe(struct pci_dev *pdev, struct nf10_card *card){
    ...
    if (err){
        printk(KERN_ERR "nf1_cml: Error allocating chrdev\n");
        return err;
    }
    ...
    if (err){
        printk(KERN_ERR "nf1_cml: Error adding /dev/nf10\n");
        return err;
    }
