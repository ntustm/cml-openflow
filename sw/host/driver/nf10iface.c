...
static netdev_tx_t nf10i_tx(struct sk_buff *skb, struct net_device *dev){
    ...
    if(skb->len > 1514){
        printk(KERN_ERR "nf1_cml: packet too big, dropping");
...
int nf10iface_probe(struct pci_dev *pdev, struct nf10_card *card){
    ...
    if(request_irq(pdev->irq, int_handler, 0, DEVICE_NAME, pdev) != 0){
        printk(KERN_ERR "nf1_cml: request_irq failed\n");
    ...
    for (i = 0; i < 4; i++){
        ...
        if(netdev == NULL){
            printk(KERN_ERR "nf1_cml: Could not allocate ethernet device.\n");
            ret = -ENOMEM;
        ...
        if(register_netdev(netdev)){
            printk(KERN_ERR "nf1_cml: register_netdev failed\n");
        }
